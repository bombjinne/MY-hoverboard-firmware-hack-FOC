...
void beepShort(uint8_t freq) {
    ...
    HAL_Delay(100);
    buzzerFreq = 0;
    HAL_Delay(80);   // audible gap between consecutive beeps
}
...
void electricBrake(uint16_t speedBlend, uint8_t reverseDir) {
    ...
    if (input2[inIdx].cmd >= 0 && input2[inIdx].cmd < ELECTRIC_BRAKE_THRES) {
      ...
      // Pass through: for go-kart, don't remap the drive range.
      // Original hoverboard code rescaled [120,1000]->[0,1000], which compressed
      // the throttle range so that 47% pedal already gave max M1 speed.
