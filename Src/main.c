...
int main(void) {
  ...
  int16_t board_temp_adcFilt  = adc_buffer.temp;
  ...
  #ifdef MULTI_MODE_DRIVE
    // Default to Mode 1 (Beginner) at startup
    drive_mode = 0;
    ...
    printf("Drive mode %i (default): max_speed:%i acc_rate:%i \r\n", drive_mode, max_speed, rate);
  #endif
  ...
  while(1) {
    ...
    #if 0 // defined(VARIANT_HOVERCAR) && defined(MULTI_MODE_DRIVE)
    {
      ...
      // --- PB10: Short press cycles speed mode 1->2->3->1 ---
      if (speed_btn == 0 && speed_btn_prev == 1 && (now - speed_btn_debounce) > KART_DEBOUNCE_MS) {
        speed_btn_debounce = now;
        ...
        switch (drive_mode) {
          ...
          case 1:
            ...
            beepShort(6); beepShort(6);   // 2 beeps = Mode 2
            break;
          case 2:
            ...
            beepShort(6); beepShort(6); beepShort(6);   // 3 beeps = Mode 3
            break;
        }
      }
      speed_btn_prev = speed_btn;

      // --- PB11: Toggle switch for reverse (LOW = reverse ON, only when stationary) ---
      if (rev_btn == 0 && rev_btn_prev == 1 && (now - rev_btn_debounce) > KART_DEBOUNCE_MS) {
        rev_btn_debounce = now;
        if (speedAvgAbs < 50) {                    // Only allow reverse when nearly stationary
          reverse_enabled = 1;
          beepLong(10);     // Long beep = reverse ON
        } else {
          beepShort(1); beepShort(1); beepShort(1); // 3 short = too fast for reverse
        }
      } else if (rev_btn == 1 && rev_btn_prev == 0 && (now - rev_btn_debounce) > KART_DEBOUNCE_MS) {
        rev_btn_debounce = now;
        reverse_enabled = 0;
        beepShort(3);     // Short beep = reverse OFF
      }
      rev_btn_prev = rev_btn;
    }
    #endif
      ...
      if (inIdx == CONTROL_ADC) {                                   // Only use use implementation below if pedals are in use (ADC input)
        ...
        electricBrake(speedBlend, reverse_enabled);             // Apply Electric Brake. reverse_enabled from PB11 toggle
      #endif
