...
```
        15V (右侧线缆红线)
         ...
         │       ├── GND                ← 喇叭电流回路走这里
         │       │
         │       └── PB11 (右侧线缆绿线)  ← 主板检测开关状态
         │
         PB10 (右侧线缆蓝线) ── 自复位按钮 ── GND    ← 独立换挡按钮
```

- 自锁开关闭合时：**喇叭响 + PB11 检测到 GND → 倒车模式**
- 喇叭的 15V 电流走 **开关 → GND**，不进主板引脚
- PB11 只检测电压（0V/3.3V），不走电流
- 倒车喇叭只在开关闭合时响，换挡时自复位按钮不影响喇叭

> 🔧 **省引脚替代方案**：去掉自复位按钮，把 PB10 也接到自锁开关上。**快速拨动（关→开→关）= 换挡**；**接通保持（关→开→保持 > 1秒）= 切换倒挡**。这样 PB11 完全空闲，右侧绿线可另作他用。
