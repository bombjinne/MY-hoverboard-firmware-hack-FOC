...
| **扩展板物理按钮** | PC14换挡 PA4倒车（需启用按钮模式） | 与RF遥控共用命令协议 |
...
| **M1** `RF_CMD_MODE1` | 0x03 | 强制切换到 M1 档 | 短鸣 3 声 |
| **停车** `RF_CMD_STOP` | 0x04 | 禁用电机 | 长鸣 1 声 |
| **换挡** `RF_CMD_NEXT_MODE` | 0x05 | 循环 M1→M2→M3→M1 | 1/2/3 声短鸣 |
| **倒车开** `RF_CMD_REVERSE_ON` | 0x06 | `backwardDrive=1` | 短+长鸣 |
| **倒车关** `RF_CMD_REVERSE_OFF` | 0x07 | `backwardDrive=0` | 长鸣 1 声 |
