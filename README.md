...
| 功能 | 操作 | 反馈 |
|---|---|---|
| **档位切换** | PB10 短暂接地（关→开→关） | 蜂鸣提示：1声=M1慢速、2声=M2中速、3声=M3快速 |
| **前进/倒车** | PB10 持续接地 > 1秒（关→开→保持） | 仅静止时生效。切换时蜂鸣提示：倒车=2短声，前进=1短声，行驶中长按=4短声警告 |

> PB10 使用钮子开关：快速 **关→开→关** = 换挡；直接拨到 **开** 保持不动 > 1秒 = 切换倒挡。PB10 持续接地是安全的（GPIO 输入模式，内部上拉电阻限流），不会重复触发倒挡。
