...
|---|---|---|---|
| 🔴 红 | **15V** | 不用，不可接大功率设备 | 15V 200mA max |
| 🔵 蓝 | **PB10** | 换挡按钮（自复位，短接→GND） | TX / SCL / PWM1 |
| 🟢 绿 | **PB11** | 倒车开关（自锁钮子开关，接通→GND 即倒车）| RX / SDA / iBUS / PPM / PWM2 |
| ⚫ 黑 | **GND** | 公共地 | GND |
...
**方案一：PB11 输出驱动继电器（推荐，需单按钮模式）**

```
PB10 (右侧蓝线) ──→ 钮子开关 ──→ GND    ← 单按钮: 快拨=换挡, 长通=倒挡

                                        ← PB11 空闲，作为输出
PB11 (右侧绿线) ──→ 5V继电器模块信号端
                     继电器模块 VCC ──→ 3.3V (AMS1117 取电)
                     继电器模块 GND ──→ GND
                     继电器 COM ──→ 15V (右侧红线)
                     继电器 NO ──→ 喇叭正极
                                  喇叭负极 ──→ GND
...
- 倒车时主板检测到反向，自动将 PB11 拉高 → 继电器吸合 → 喇叭响
- 15V 电流只走继电器触点，完全不碰主板
- 5V 继电器模块由 3.3V 供电（大多数 5V 继电器 3.3V 也能吸合，否则用 5V 降压模块）

**方案二：不改代码、不动引脚，独立喇叭回路**

```
15V ──→ 12V降压模块 ──→ 喇叭正极
                         喇叭负极 ──→ 钮子开关另一组触点 ──→ GND

PB11 (右侧绿线) ──→ 钮子开关（和喇叭同轴，只检测不供电）
```

用一个**双刀钮子开关（DPDT）**，一组触点接喇叭，一组触点接 PB11，互不干扰。
---
...
| 功能 | 操作 | 反馈 |
|---|---|---|
| **档位切换** | PB10 自复位按钮短接→GND | 蜂鸣提示：1声=M1慢速、2声=M2中速、3声=M3快速 |
| **前进/倒车** | PB11 自锁钮子开关接通→GND | 仅静止时生效。倒车限速 M1（最慢档） |
...
| **坡道辅助** | 坡道停车会防止溜车 | 会增加耗电量 |

> ⚙️ **替代方案：单按钮模式** 如果想把 PB11 省出来做其他功能，可用一个按钮（PB10）同时控制换挡和倒车：**快速拨动 PB10（关→开→关）= 换挡**；**持续接通 PB10（关→开→保持 > 1秒）= 切换倒挡**（仅静止有效）。这样 PB11 即可空闲。
...
| `ELECTRIC_BRAKE_THRES` | 120 | 动能回收介入阈值 |
...
> 每次上电默认启动 M2（中级模式），可通过 PB10 按钮循环切换 M1 ↔ M2 ↔ M3。
...
| `SUPPORT_BUTTONS_RIGHT` | 已启用 | 右侧传感器线缆按钮（PB10=换挡，PB11=倒车） |
...
| `INACTIVITY_TIMEOUT` | 8 分钟 | 无人操作自动关机 |
